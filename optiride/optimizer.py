...
def pace_heuristic(dist_m: np.ndarray, slope: np.ndarray, bearings_deg: np.ndarray, rb: RiderBike, env: Environment,
                   P_flat: float, up_mult: float = 1.10, down_mult: float = 0.75,
...
def simulate(dist_m: np.ndarray, slope: np.ndarray, bearings_deg: np.ndarray, P_target: np.ndarray,
             rb: RiderBike, env: Environment):
    ds = dist_m[1]-dist_m[0]
